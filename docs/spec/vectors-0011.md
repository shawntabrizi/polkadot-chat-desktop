...
2026-09-24 with a throwaway script (SCALE by hand, keyed BLAKE2b-256 from
`@noble/hashes` 2.4.0 `blake2b(payload, { key, dkLen: 32 })`, AES-256-GCM from
Node `crypto`). The pca codec and the desktop codec must each reproduce these
bytes before they are pinned in `bot-core/test/codec.test.mjs` and
`content.spec.ts`. A disagreement is a bug in this file until proven otherwise.
...
The sealed `state` statement uses AAD `b"grp" : signer : u32 epoch : 01`; its
ciphertext vector is TODO (same method as (b)).
...
- A real `K(A, B)` from two P-256 keys (Appendix A HKDF) instead of the stand-in.
- Sealed `state`, a full `Rekey` with two entries, `history`, `joinDecision`,
  `keyRequest`.
- A full `GroupStatement` with expiry, channel, topic and an Sr25519 proof.
