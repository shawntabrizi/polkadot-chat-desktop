...
Correction (2026-09-24): the phones do not diverge from the spec. chat-spec main (`134cad7`) merged RFC-0004 (ChaCha20-Poly1305) and RFC-0001 (the versioned `VersionedUploadedFile` root) on 2026-07-31, and the phones follow both. This review read a stale copy (`7af4fab`). The divergence is between the old `base-spec.md` body text and the RFCs. The flag for chat-spec is now `docs/upstream/12-hop-cipher-envelope.md`: fold the RFCs into the base text.
