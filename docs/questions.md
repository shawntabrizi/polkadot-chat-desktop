...
- **Fleet bots and files.** Kind 250 is no longer sent, so a fleet bot still on pre-M20 pca (the fleet ran 289d02c on 2026-09-24) gets photos over HOP in the phones' dialect, which it cannot read ("I can't access that image"). Deploy pca M20 (b8b9fc4) to the fleet? The desktop does not touch the fleet.
- **`//Eve` refused even 8 MiB on 2026-09-24** (`InsufficientAuthorizerBudget`, live in e2e:caps). A fresh test identity then has no Bulletin authorization, so it can neither store (Bulletin) nor submit to a HOP node (1012). Only `pcde2e` still has one (45 of 64 MB, until 2026-10-08). Can the devnet authorizer budget be refilled, or is there another devnet authorizer?
