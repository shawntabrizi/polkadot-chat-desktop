...
timeout 900 npm run package >/tmp/m3-package.log 2>&1 || { tail -30 /tmp/m3-package.log; fail "package failed"; }
ls dist/*.dmg >/dev/null 2>&1 || fail "no dmg in dist/"
out=$(timeout 120 npm run smoke:packaged 2>&1 | tail -5) || true
